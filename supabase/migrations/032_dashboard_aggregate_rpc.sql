...
--   2. GET /rest/v1/wishlists?household_id=…&visibility=…  (shared)
--
-- AFTER:  1 RPC call
--   POST /rest/v1/rpc/get_dashboard_summary
--
-- Benchmark notes (estimated, based on typical Supabase latency):
--   Before: ~2 × 20–50 ms = 40–100 ms elapsed (parallel, but 2 TCP connections)
--   After:  ~1 × 25–60 ms = 25–60 ms elapsed (single server-side join)
--   Expected reduction: ~35–50 % in total dashboard load latency
--
-- Security guarantees:
...
begin
  ...
  raise notice '   Reduces dashboard round-trips from 2 → 1';
end $$;
